      ...
      <PopoverTrigger asChild>
        <Button
          ...
          <span className="truncate">
            {value.length > 0
              ? options
                  .filter((o) => value.includes(o[selector || "name"]))
                  .map((o) => o.name)
                  .join(", ")
              : placeholder}
          </span>
      ...
      <PopoverContent className="w-full p-0">
        <Command>
          ...
          <CommandGroup className="max-h-64 overflow-auto">
            {options.map((option) => {
              const isSelected = value.includes(option[selector || "name"]);
              return (
                <CommandItem
...
const AddProductModal = ({ open, onOpenChange, onProductAdded }) => {
  ...
  useEffect(() => {
    const customSizes = selectedCustom
      ...
      .map((size) => size.trim())
      .filter(Boolean);
    const sizes = [...selectedLetters, ...selectedNumbers, ...customSizes];
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="!max-w-4xl max-h-[90vh] overflow-y-auto w-6xl">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold">Add Product</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleFormSubmit} className="space-y-8 p-1">
