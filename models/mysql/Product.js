...
export const Product = {
  ...
  async findAll(options = {}) {
    ...
    const isActive = filters.is_active !== undefined ? filters.is_active : (options.isActive !== undefined ? options.isActive : true);
    ...
    if (categoryId) {
      ...
      countQuery += ' AND p.id IN (SELECT product_id FROM product_categories WHERE category_id = ?)';
  ...
  async create(data) {
    ...
    try {
      ...
      if (data.categories && data.categories.length > 0) {
        const categoryValues = data.categories.map(catId => [productId, catId]);
  ...
  async updateById(id, updateData) {
    ...
    if (updateData.description) { fields.push('description = ?'); values.push(updateData.description); }
    if (updateData.basePrice) { fields.push('base_price = ?'); values.push(updateData.basePrice); }
    if (updateData.variantOptions) { fields.push('variant_options = ?'); values.push(JSON.stringify(updateData.variantOptions)); }
    if (updateData.variantVariables) { fields.push('variant_variables = ?'); values.push(JSON.stringify(updateData.variantVariables)); }
    if (updateData.mainImages) { fields.push('main_images = ?'); values.push(JSON.stringify(updateData.mainImages)); }
    if (updateData.specifications) { fields.push('specifications = ?'); values.push(JSON.stringify(updateData.specifications)); }
    if (updateData.tags) { fields.push('tags = ?'); values.push(JSON.stringify(updateData.tags)); }
    if (updateData.isActive !== undefined) { fields.push('is_active = ?'); values.push(updateData.isActive); }
    if (updateData.averageRating !== undefined) { fields.push('average_rating = ?'); values.push(updateData.averageRating); }
    if (updateData.totalReviews !== undefined) { fields.push('total_reviews = ?'); values.push(updateData.totalReviews); }
    if (updateData.totalSold !== undefined) { fields.push('total_sold = ?'); values.push(updateData.totalSold); }
  ...
  _parseProduct(row) {
    return {
      ...
      variantOptions: typeof row.variant_options === 'string' ? JSON.parse(row.variant_options) : row.variant_options,
      variantVariables: typeof row.variant_variables === 'string' ? JSON.parse(row.variant_variables) : row.variant_variables,
      mainImages: typeof row.main_images === 'string' ? JSON.parse(row.main_images) : row.main_images,
