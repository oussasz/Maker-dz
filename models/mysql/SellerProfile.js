...
export const SellerProfile = {
  ...
  async findByShopName(shopName) {
    const pool = await getConnection();
    const [rows] = await pool.query(
      "SELECT * FROM seller_profiles WHERE shop_name = ?",
      [shopName],
    );
    return rows[0] || null;
  },
  ...
  async updateByUserId(userId, updateData) {
    ...
    if (updateData.shopDescription !== undefined) {
      fields.push("shop_description = ?");
      values.push(updateData.shopDescription);
    }
    if (updateData.shopLogo !== undefined) {
      fields.push("shop_logo = ?");
      values.push(updateData.shopLogo);
    }
    if (updateData.shopBanner !== undefined) {
      fields.push("shop_banner = ?");
      values.push(updateData.shopBanner);
    }
