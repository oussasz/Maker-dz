...
export const Cart = {
  ...
  async getCartItems(cartId) {
    ...
    return rows.map((item) => ({
      ...
      variant: {
        ...
        images:
          typeof item.variant_images === "string"
            ? JSON.parse(item.variant_images)
            : item.variant_images,
      },
    }));
  },
  ...
  async clear(userId) {
    ...
    await pool.query("UPDATE carts SET total_amount = 0 WHERE id = ?", [
      cart.id,
    ]);
  ...
  async updateTotal(cartId) {
    ...
    await pool.query("UPDATE carts SET total_amount = ? WHERE id = ?", [
      total,
      cartId,
    ]);
